...
class openstack-hyper-v::base::virtual_switch(
  ...
  if $enable {
    ...
    exec { 'update-notes':
      ...
      unless   => "if ((Get-VMSwitch \"${name}\").Notes -ne \"${notes}\") { exit 1 }",
      provider => powershell,
    ...
    exec { 'update-switch-type':
      ...
      unless   => "exit (Get-VMSwitch \"${name}\").SwitchType -ne ${switch_type}",
      provider => powershell,
      require  => Exec['create-vm-switch'],
    }
    ...
    if $switch_type == 'External' {
      exec {'update-interface':
        ...
        unless   => "${get_adapter} exit ((Get-VMSwitch \"${name}\").NetAdapterName -ne \$if.ifAlias)",
        provider => powershell,
        require  => Exec['create-vm-switch'],
      }
      ...
      exec {'update-os-management':
        ...
        unless   => "exit ((Get-VMSwitch \"${name}\").AllowManagementOS -ne \$${os_managed})",
        provider => powershell,
        require  => Exec['create-vm-switch'],
      }
    }
